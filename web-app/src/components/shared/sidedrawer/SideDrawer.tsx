...
function SideDrawer({
  ...
  const isActiveRoute = (route: string) => {
    return useLocation().pathname === route;
  };

  const getActiveStyle = (route: string) => {
    if (isActiveRoute(route)) {
      return 'pr-4 bg-gradient-to-r from-emerald-600/50 to-emerald-500/20 text-white font-semibold rounded-r-lg shadow-md';
    }
  };
      ...
      <div className="flex flex-row">
        ...
        >
          <ul className="flex flex-col justify-center items-start mt-10 space-y-14 text-gray-100 font-semibold tracking-wider">
            <Link
              className={`relative flex w-full h-14 items-center space-x-4 pl-4 ${getActiveStyle('/')}`}
              to="/"
            >
              {isActiveRoute('/') && (
            ...
            <Link
              className={`relative flex w-full h-14 items-center space-x-4 pl-4 ${getActiveStyle('/testplans')}`}
              to="/testplans"
            >
            ...
            <Link
              className={`relative flex w-full h-14 items-center space-x-4 pl-4 ${getActiveStyle('/testcases')}`}
              to="/testcases"
            >
          ...
          <div className="flex flex-row justify-center m-auto items-center w-full h-14 space-x-4 text-gray-100 text-lg font-semibold tracking-wider">
